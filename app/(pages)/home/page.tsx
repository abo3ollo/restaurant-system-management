...
import { ShieldCheck, DollarSign, UtensilsCrossed, ArrowLeft } from "lucide-react";
...
export default function FoodicsHome() {
    ...
    const handleContinue = async (roleKey: Role) => {
        ...
        try {
            ...
            const route = ROLES.find(r => r.key === roleKey)?.route;
            if (route) {
                window.location.href = route; // ← hard navigate after upsert
            }
        } catch (error) {
    ...
    // Loading while Convex fetches user
                                    ...
                                    {ROLES.map((r) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={cn("flex items-center gap-4 px-5 py-4 rounded-2xl border-2 transition-all duration-150 group text-left cursor-pointer", r.bg)}
                                            >
                        ...
                        {selectedRole && role && (
                            <div>
                                ...
                                {isSignedIn ? (
                                    ...
                                    >
                                        <role.icon size={15} />
                                        {isLoading ? "Loading..." : `Continue as ${role.label}`}
                                    </button>
                                ) : (
                                    <SignInButton mode="modal">
                                        <button className={cn("w-full h-12 rounded-xl text-sm font-black tracking-wide text-white shadow-lg flex items-center justify-center gap-2", role.btnClass)}>
                                            <role.icon size={15} />
